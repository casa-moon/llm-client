        ...
        match command {
            ...
            "file" => {
                ...
                let path_abs = if path_str.starts_with("http://") || path_str.starts_with("https://") {
                    ...
                    match std::fs::canonicalize(path) { Ok(p) => p, Err(_) => path.to_path_buf() }
                };
                if !path_abs.is_file() { println!("\nFile not found.\n"); continue; }
                session.append_message_to_file("\n\n***\n\n### User:\n")?;
            ...
            "dir" => {
                let path_str = Text::new("Enter path:").prompt()?;
                let path = std::path::Path::new(&path_str);
                let path_abs = match std::fs::canonicalize(path) { Ok(p) => p, Err(_) => path.to_path_buf() };
                if !path_abs.is_dir() { println!("\nDirectory not found.\n"); continue; }
...
fn estimate_image_tokens(url: &str, session: &mut ChatSession) -> Result<()> {
    ...
    if let Ok(mut rdr) = reader { if let Ok(img) = rdr.decode() {
