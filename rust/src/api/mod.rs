...
impl ApiClient for OpenAIClient {
    ...
    fn send_message(&mut self, model: &str, log: &MessageLog) -> Result<String> {
        ...
        struct ChatChoiceMsg { content: String }
        #[derive(serde::Deserialize)]
...
impl ApiClient for AnthropicClient {
    ...
    fn send_message(&mut self, model: &str, log: &MessageLog) -> Result<String> {
        ...
        #[derive(serde::Deserialize)]
        struct ContentItem { text: Option<String> }
        #[derive(serde::Deserialize)]
        struct AnthResp { content: Vec<ContentItem> }
...
pub struct GoogleClient { pub api_key: String }
...
pub struct PerplexityClient { pub api_key: String }
...
pub struct MistralClient { pub api_key: String }
    ...
    match tmpl {
        TemplateKey::OpenAI => {
            let msgs: Vec<serde_json::Value> = raw
                .iter()
                .map(|m| {
                    let role = match m.role { Role::Model => "assistant", Role::User => "user" };
                    match m.kind {
