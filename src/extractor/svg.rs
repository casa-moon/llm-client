...
pub fn rasterize_svg_to_png_b64(
  session: &ChatSession,
  svg_bytes: &[u8],
  ...
  let tree = usvg::Tree::from_data(svg_bytes, &opt).map_err(|_| anyhow!("Failed to parse SVG"))?;

  let size = tree.size().to_int_size();
  let (w, h) = (size.width(), size.height());
  if w == 0 || h == 0 {
    return Err(anyhow!("Invalid SVG size"));
  }

  // Render with resvg into a pixmap
  let mut pixmap =
    tiny_skia::Pixmap::new(w, h).ok_or_else(|| anyhow!("Failed to create pixmap"))?;
